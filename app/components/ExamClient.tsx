...
export default function ExamClient({ exam }: { exam: Exam }) {
  ...
  const PAPER_WIDTH = 'w-[90%] max-w-2xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-5xl min-[2000px]:max-w-6xl'
  ...
  return (
    <div className={`relative min-h-screen font-sans bg-stone-100 ${blocked ? 'overflow-hidden' : ''}`}>
      ...
      <div className={blocked ? 'blur-sm pointer-events-none select-none opacity-60' : ''}>
        ...
        {/* Floating sticky nav */}
        {started && (
          <div className="sticky top-3 z-10 flex justify-center px-4 print:hidden">
            <div className={`${PAPER_WIDTH} bg-stone-50/90 backdrop-blur-md shadow-md border border-stone-200/80 rounded-2xl px-6 py-3`}>
              <div className="flex justify-between items-center">
                <h2 className="text-base font-bold text-gray-900 truncate mr-4">{exam.title}</h2>
                <div className="flex items-center gap-4 flex-shrink-0">
                  <span className="text-sm text-gray-500">{answeredCount} / {exam.questions.length}</span>
        ...
        {/* Paper */}
        <div className={`${PAPER_WIDTH} mx-auto py-8 px-0 print:p-0 print:max-w-none print:w-full`}>
          <div className="bg-stone-50 shadow-sm border border-stone-200/70 px-8 sm:px-12 py-10 print:shadow-none print:border-none print:px-0 print:py-0">

            {!started && (
              <div className="pb-3 border-b border-gray-200 mb-8">
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-bold text-gray-900">{exam.title}</h2>
                  <span className="text-sm text-gray-500">0 / {exam.questions.length} answered</span>
                </div>
                <div className="mt-2 h-1 bg-gray-200 rounded-full" />
              </div>
            )}
            ...
            {started && !review && (
              <div className="text-center pt-4 pb-6">
                <button
                  ...
                  className="bg-gray-900 text-white py-3.5 px-9 rounded-lg text-base cursor-pointer hover:bg-gray-700 transition-colors"
                >
      ...
      {review && !timeUpAlert && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex gap-2 bg-stone-50/90 backdrop-blur-md border border-stone-200/80 shadow-lg rounded-2xl px-3 py-3 print:hidden">
          <button
